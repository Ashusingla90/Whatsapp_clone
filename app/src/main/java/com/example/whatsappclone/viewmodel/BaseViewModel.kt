...
import com.example.whatsappclone.presentation.homescreen.ChatListModel
...
class BaseViewModel : ViewModel() {
    ...
    fun getChatForUser(userId: String, callback: (List<ChatListModel>) -> Unit) {
        ...
        chatRef.orderByChild("userId").equalTo(userId)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onCancelled(error: DatabaseError) {
                    Log.e("BaseViewModel", "Error fetching users chats: ${error.message}")
                    callback(emptyList())
                }
            })
    }
    ...
    private fun loadChatData() {
        ...
        if (currUserId != null) {
            ...
            chatRef.orderByChild("userId").equalTo(currUserId)
    ...
    fun addChat(newChat: ChatListModel) {
        ...
        if (currUserID != null) {
            ...
            val chatWithUser = newChat.copy(name = currUserID)
