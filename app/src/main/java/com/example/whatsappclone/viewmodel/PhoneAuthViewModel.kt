...
class PhoneAuthViewModel @Inject constructor(
    ...
    // database ma jaha pr jaka data store hoga
        ...
        firebaseAuth.signInWithCredential(credential)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    _authState.value = AuthState.Error(task.exception?.message ?: "Sig-in Failed")
    ...
    fun verifyCode(otp: String, context: Context) {
        ...
        val currAuthState = _authState.value

        if (currAuthState !is AuthState.CodeSent || currAuthState.verificationId.isEmpty()) {
            Log.e("PhoneAuth", "Attempting to verify OTP wihtout a valid verification ID")

            _authState.value = AuthState.Error("Verification not started or invalid ID")
            return
        }
        val credential = PhoneAuthProvider.getCredential(currAuthState.verificationId, otp)
        siginWithCredential(credential, context)
    }

    fun saveUserProfile(uid: String, name: String, status: String, profileImage: Bitmap?) {
        ...
        val userProfile = PhoneAuthUser(
            ...
            profileImage = encodedImage as String?
        )
